...
describe('Check Route', () => {
    ...
    it('GET request with token', async () => {
        const res = await request(app).get('/api/mandataires').set({ 'auth-token': member_token })
        expect(res.status).toEqual(200);
    });

    it('POST request without token', async () => {
        const res = await request(app).post('/api/mandataires').send(mandataire)
        expect(res.status).toEqual(500)
    })

    it('POST request with member role', async () => {
        const res = await request(app).post('/api/mandataires').send(mandataire).set({ 'auth-token': member_token })
    ...
    it('POST request with admin role and valid object', async () => {
        const res = await request(app).post('/api/mandataires').send(mandataire).set({ 'auth-token': admin_token })
        expect(res.status).toEqual(200)
    })

    it('POST request with admin role and invalid Object', async () => {
        const res = await request(app).post('/api/mandataires').send(mandatairePartial).set({ 'auth-token': admin_token })
        expect(res.status).toEqual(500)
    })
});